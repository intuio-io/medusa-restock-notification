 ...
 */
    async getOrCreateList(variantId, variantTitle) {
        try {
            ...
            const [response] = await this.sendgridClient_.request(requestGet);
            const existingList = response.body.result?.find(list => list.name === `${variantTitle}-${variantId}`);
            if (existingList) {
            ...
            const [createResponse] = await this.sendgridClient_.request(requestCreate);
            return createResponse.body.id;
        }
        catch (error) {
            this.logger_.error("SendGrid List Error:", error);
            throw new utils_1.MedusaError(utils_1.MedusaError.Types.INVALID_DATA, "Failed to manage SendGrid list");
        }
    }
    /**
 ...
 */
    ...
    * Remove subscription
    */
    async removeSubscription(variantId, email) {
        try {
            // Get contact ID
            const contactId = await this.getContactId(email);
            if (!contactId) {
                throw new utils_1.MedusaError(utils_1.MedusaError.Types.NOT_FOUND, `No contact found for email ${email}`);
            }
            const variant = await this.productVariantService_.retrieve(variantId, {
                relations: ["product"]
            });
 ...
 */
    async removeSubscriptionByListId(email, listId) {
        try {
            // Get contact ID
            const contactId = await this.getContactId(email);
            if (!contactId) {
     ...
     */
    async listSubscriptions(email, limit = 10, offset = 0) {
        try {
            // Get contact details
            const contactId = await this.getContactId(email);
            if (!contactId) {
            ...
            const [listsResponse] = await this.sendgridClient_.request({
                url: `/v3/marketing/lists`,
                method: 'GET',
                qs: {
                    page_size: limit,
            ...
            const [contactResponse] = await this.sendgridClient_.request({
            ...
            const variants = await Promise.all(variantIds.map(id => this.productVariantService_.retrieve(id, {
                relations: ["product"]
            }).catch(err => null)));
            // Combine data
